...
- **utils**: add `slugify` utility and update exports for string manipulations (a343e5e)
...
- **utils**: enhance `createURI` to support optional auth and add comprehensive tests (24813d6)
...
- **utils**: simplify RabbitMQ reply handling by removing `pendingReplies` tracking (a69be8d)
...
- **utils**: implement RabbitMQ `direct reply-to` optimization for RPC calls (7493266)
