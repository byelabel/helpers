import { isFunction, isNonEmptyString, isNonNullObject, isUndefined } from './validator';

type EnvVars = Record<string, string | undefined>;

const isNodeRuntime = !isUndefined(process) && isNonEmptyString(process?.versions?.node) && isFunction(process?.cwd);
    ...
    if (existsSync(filePath)) {
      ...
      if (env.error) {
        ...
        if (cluster.isPrimary) {
          console.info(`Environment variables loaded: "${envFile}"`);
        }
      }
    }
  } else if (!isUndefined(process) && isNonNullObject(process?.env)) {
